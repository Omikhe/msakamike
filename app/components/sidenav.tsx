      ...
      <div className="flex flex-col gap-14 w-[82px] items-center justify-center">
        ...
        <nav>
          <div className="p-1 bg-[#161616] border-[#202020] border-2 rounded-full">
            <div className="flex lg:flex-col gap-2 bg-gradient-to-b from-[#282828] to-[#1B1B1B] rounded-full">
              {navLinks.map((link) => {
                ...
                return (
                  <div key={link.href} className="relative group">
                    <Link
                      href={link.href}
                      className={`m-1 p-2.5 lg:p-2 gap-2 flex justify-center items-center rounded-full transition-all duration-200 ${isActive
                    ...
                    >
                      {link.icon}
                      <div className={`lg:hidden ${isActive ? "flex" : "hidden"}`}>{link.label}</div>

                    </Link>

                    {/* Tooltip */}
                    <div className="hidden lg:block absolute left-full ml-3 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none">
